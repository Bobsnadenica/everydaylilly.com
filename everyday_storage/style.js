...
// init canvas
  ...
  document.body.prepend(canvas); // ensures canvas is behind other elements
  ...
  // --- helpers: math, vectors, easing, noise ---
  const PI = Math.PI;
  ...
  class Vector {
    ...
    setMag(m) {
      const cur = Math.hypot(this.x, this.y) || 1e-9;
      this.x = (this.x / cur) * m;
      this.y = (this.y / cur) * m;
      return this;
    }
  ...
  const Util = {
    ...
    threeAngle(a, b, c) {
      ...
      const mag2 = Math.hypot(cbx, cby) || 1e-9;
  ...
  // Simple pseudo-noise with seeding for stability
  const noise = (function() {
    let _seed = Math.random() * 1e9;
    function seed(s) { _seed = s * 1e6; }
    function fract(x) { return x - Math.floor(x); }
    function hash(x, y) {
      return fract(Math.sin(x * 127.1 + y * 311.7 + _seed) * 43758.5453123);
    }
    ...
    return {
      seed,
      simplex2(x, y) { return smoothNoise(x, y) * 2 - 1; } // [-1, 1]
    };
  })();
  // --- end helpers ---
