...
- 2026-06-16 root hero update: `index.html` and `index-bg.html` now expose `data-gallery-domain` and a `data-gallery-hero` header. `style.css` owns the polished hero fallback background, glass content panel, explicit white hero text, and responsive carousel cleanup. `script.js` uses the existing Cognito session plus a short per-user cached gallery manifest to pick a random signed `heroPhotos`/`photos` image as the hero background. If no session, no signed image, or preload fails, the local `images/hero.jpg` fallback remains.
- 2026-06-16 root hero validation: `node --check script.js` and `git diff --check` passed. Browser QA used `http://localhost:8000/index.html?v=hero-final-20260616` plus a temporary mock at `http://127.0.0.1:8001/everyday-lilly-hero-mock.html`; desktop `1280x720` and mobile `390x844` had no horizontal overflow, the real fallback used `images/hero.jpg`, carousel `#carousel-next` advanced from `lilly` to `dandelion`, and the mock signed manifest swapped the hero to `months/hero/7/mock-hero.svg`. The only console warning seen was the known Tailwind CDN production warning.
